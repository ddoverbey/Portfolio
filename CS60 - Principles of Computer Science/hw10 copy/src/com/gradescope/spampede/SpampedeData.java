...
class SpampedeData {
	...
	// moves snake without making snake longer when there is no spam in front of it
	
	public void moveSnakeNoSpam() {
		BoardCell newHead = this.getNextCellInDir();
		BoardCell oldHead = this.getSnakeHead();
		oldHead.becomeBody();
		this.snakeCells.addLast(newHead);
		newHead.becomeHead();
		BoardCell oldTail = this.snakeCells.removeFirst();
		oldTail.becomeOpen();
	}
	
	/*
	 * moves snake but lets it eat spam and grow
	 */
	
	public void moveSnakeWithSpam() {
		BoardCell newHead = this.getNextCellInDir();
	...
	public void reverseDirection() {
		...
		Iterator<BoardCell> iter = snakeCells.iterator();
		
		while(iter.hasNext()) {
			revSnake.addFirst(iter.next());
		}
		
		this.snakeCells = revSnake;
		this.snakeCells.getLast().becomeHead();
		
		BoardCell neck = this.getSnakeNeck();
		if (this.getNorthNeighbor() == neck) {
			this.setDirectionSouth();
		}
		
		else if(this.getSouthNeighbor()== neck) {
			this.setDirectionNorth();
		}
		
		else if(this.getEastNeighbor() == neck) {
