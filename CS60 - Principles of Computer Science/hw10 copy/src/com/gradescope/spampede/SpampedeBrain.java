...
public class SpampedeBrain extends SpampedeBrainParent {
	 ...
	 *  <p>
	 *  Called by test_eatspam(), test_noeatspam(), cycle()
	 */
	void updateSnake() {
	...
	public void advanceTheSnake(BoardCell nextCell) {
		...
		if (nextCell.isWall() || nextCell.isBody()) {
			...
			this.theData.moveSnakeWithSpam();
			
			
		} else {
			this.theData.moveSnakeNoSpam();
	...
	public BoardCell getNextCellFromBFS() {
		// Initialize the search.
				...
				while(cellsToSearch.isEmpty() == false) {
					BoardCell dqNode = cellsToSearch.remove();
					
					for(BoardCell nextCell:this.theData.getNeighbors(dqNode)) {
						if(nextCell.inSearchListAlready() == false && nextCell.isOpen()) {
							cellsToSearch.add(nextCell);
							nextCell.setAddedToSearchList();
							nextCell.setParent(dqNode);;
							if(nextCell.isSpam()) {
								closestSpamCell = nextCell;
								return this.getFirstCellInPath(nextCell);
							}
						}
					}
				}
