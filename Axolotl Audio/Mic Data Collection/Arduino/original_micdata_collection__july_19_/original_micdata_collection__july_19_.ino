...
const unsigned long sampleIntervalMicros = 50; // Time interval between samples in microseconds
...
void loop() {
  ...
  File dataFile = SD.open("data42.txt", FILE_WRITE);
  if (!dataFile) {
  ...
  for (unsigned long sampleCount = 0; sampleCount < numSamples; sampleCount++) {
    ...
    int sensorValue = analogRead(analogPin);  // Read analog value from A1
